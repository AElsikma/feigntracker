...
// Rol veritabanı (Oyun kurallarına göre kategorize edildi)
const roleDB = {
...
cards.forEach(card => {
  card.addEventListener('click', () => {
    ...
    if (pendingAction && activePlayer) {
      ...
      // TUZAĞA YAKALANMA BUTONLARI STANDART BOYUT VE RENGİNE GETİRİLDİ
...
function showDecision(title, content, buttonsHTML) {
  ...
  document.getElementById('decision-content').innerHTML = content; /* innerText yerine innerHTML kullanıldı */
...
window.showDedektifTeamSelection = function(step) {
  ...
  // Dedektifin tuzağa yakalanma ihtimali de esnek tasarıma uyarlandı
